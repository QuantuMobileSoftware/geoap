...
def run_notebook(notebook_path, nb_kwargs=None, suffix='-out',
                 ...
                 return_nb=True, add_timestamp=True):
...
if __name__ == '__main__':
    ...
    folder = Path('/home/jovyan/work')
    output_dir = 'output_notebooks/'
    START_DATE = str(date.today())

    descr = """\
     Execute notebook in a folder saving the result in the "out"
     subfolder.
     """
    parser = argparse.ArgumentParser(description=descr, epilog='\n')
    parser.add_argument('notebook_path', help='notebook path to be executed.')
    
    msg = 'max execution time (seconds) for each cell before the execution is aborted.'
    parser.add_argument('--timeout', type=int, default=3600, help=msg)
    
    msg = 'Name of kernel executing the notebook.\n Use `jupyter kernelspec list` for a list of kernels.'
    parser.add_argument('--kernel', metavar='KERNEL_NAME', default=None, help=msg)
    
    msg = 'AOI polygon as WKT string'
    parser.add_argument('--aoi', type=str, default=None, help=msg)
    
    msg = 'if True, save the output notebook in ipynb format.\n Default True.'
    parser.add_argument('--save_ipynb', type=bool, default=True, help=msg)
    ...
    # If argument `save_html` is False this argument is ignored.
    # """
    # parser.add_argument('--out_path_html', type=str, default=None, help=msg)
    
    msg = 'request id from db.'
    parser.add_argument('--request_id', type=int, default=None, help=msg)
    ...
    out_path = Path(folder, output_dir)
    notebook_path = Path(args.notebook_path)

    notebook_relative_path = Path(notebook_path.parent).relative_to(folder)
    notebook_relative_path_parts = list(notebook_relative_path.parts)
    notebook_relative_path_parts[0] = out_path

    output_path_parent = Path(*notebook_relative_path_parts)
    output_notebook_name = f'{START_DATE}_{args.request_id}_{notebook_path.name}'

    out_path_ipynb = Path(output_path_parent, output_notebook_name)
    
    if not output_path_parent.exists():
        output_path_parent.mkdir(exist_ok=True, parents=True)

    run_notebook(args.notebook_path,
                 out_path_ipynb=out_path_ipynb,
                 kernel_name=args.kernel,
                 timeout=args.timeout,
                 nb_kwargs=nb_kwargs,
                 save_ipynb=args.save_ipynb
                 )
