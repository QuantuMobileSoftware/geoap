...
class SentinelDownload:
    ...
    def download(self, tiles_dates_list):
        success_count = 0
        start_time = time.time()
        for idx, tile in tiles_dates_list.iterrows():
            tile_id = tile['tileID']
            img_date = str(tile['img_date'])
            logger.info(f"Tile number {idx} out of {len(tiles_dates_list)}, "
                        f"tile id - {tile_id}, "
            ...
            # ! This will search images in GCP directory around specified date (+-2 days).
                ...
                for date_bias in range(-2, 3, 1):
                    ...
                    if self._retrieve_images(links, img_date, tile_id):
                        ...
                        break
        end_time = time.time()
        logger.info(f'Downloaded: {success_count}/{len(tiles_dates_list)}')
