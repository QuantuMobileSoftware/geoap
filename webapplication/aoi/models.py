...
class AoI(models.Model):
    USER_DEFINED_TYPE = 1
    FIELD_TYPE = 2
    AREA_TYPE_CHOICES = ((USER_DEFINED_TYPE, "AREA"), (FIELD_TYPE, "FIELD"))
    ...
    name = models.CharField(
        max_length=200, blank=False, null=False, unique=True, verbose_name="AOI name"
    )
...
class JupyterNotebook(models.Model):
    name = models.CharField(
        ...
        null=False,
        unique=True,
        verbose_name="Notebook name",
    )
    image = models.CharField(max_length=400, verbose_name="Image")
    path = models.CharField(
        max_length=200, unique=True, verbose_name="Path to a notebook"
    )
    kernel_name = models.CharField(
        max_length=200, null=False, blank=False, verbose_name="Kernel name"
    )
    options = JSONField(
        blank=True, null=True, verbose_name="Additional container options"
    )
    ...
    additional_parameter = models.CharField(
        max_length=50, null=True, blank=True, verbose_name="Additional parameter"
    )
    run_on_gpu = models.BooleanField(
        default=True, verbose_name="Whether GPU is needed for a notebook to run"
    )
    period_required = models.BooleanField(
        default=True, verbose_name="Start and end dates are required"
    )
...
class Input(models.Model):
    ...
    assigned_from = models.CharField(
        max_length=20,
...
class Component(models.Model):
    ...
    command = models.CharField(
        max_length=1000, null=True, blank=True, verbose_name="Command"
    )
    inputs = models.ManyToManyField(Input)
    outputs = models.ManyToManyField(Output)
    require_GPU = models.BooleanField(
        default=False, verbose_name="Whether GPU is needed for a component to run"
    )
    run_validation = models.BooleanField(default=False, verbose_name="Run validation")
    success = models.BooleanField(default=False, verbose_name="Validation succeeded")
...
class Request(models.Model):
    user = models.ForeignKey(
        "user.User", on_delete=models.PROTECT, verbose_name="User id"
    )
    aoi = models.ForeignKey(
        AoI, null=True, on_delete=models.SET_NULL, verbose_name="AOI id"
    )
    ...
    error = models.CharField(
        max_length=400, blank=True, null=True, verbose_name="Error"
    )
    polygon = models.PolygonField(spatial_index=True, verbose_name="Polygon")
    additional_parameter = models.CharField(
        max_length=50, null=True, blank=True, verbose_name="Additional parameter"
